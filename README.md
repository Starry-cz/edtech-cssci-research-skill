...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/28-研究参考模块-2563eb.svg" alt="28 个研究参考模块" />
...
| 摘要要像投稿摘要，但担心承诺超过数据和设计 | [摘要论证卡](assets/abstract-argument-card-template.md) | 先核对问题、对象、设计、锁定模型、最小发现、贡献与边界，再决定能否生成连续摘要 |
...
| **发布门先于定稿** | 用“阻断—探索性可写—投稿就绪”决定当前可生成的文本 | 发现测试集选模或泄漏后仍写最终摘要、最优模型与机制结论 |
...
若存在测试集选模、泄漏、准则污染、结果不可复核或标题强度越界，不要生成投稿摘要；请给重跑步骤和题目—摘要—图表—结论的连锁更新。只有投稿就绪时才给一段连续的中文投稿摘要。
材料：
[粘贴摘要、方法摘要或分析说明]
```
...
先核验目标期刊是否明确要求结构式摘要；若没有，不在清洁稿中保留“目的/方法/结果/结论”等构思标签。检查题目强度是否匹配研究设计，预测结果是否被误写为因果效应，SHAP/ALE 是否被拼成未经观测的过程闭环。请分开输出：①可直接使用的清洁摘要；②修改说明、证据风险与待核验事项。不要把 RQ 编号、模式名、诊断标签或占位符写进摘要。
```
