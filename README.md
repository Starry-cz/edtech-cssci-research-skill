...
先核验目标期刊是否明确要求结构式摘要；若没有，不在清洁稿中保留“目的/方法/结果/结论”等构思标签。检查题目强度是否匹配研究设计，预测结果是否被误写为因果效应，SHAP/ALE 是否被拼成未经观测的过程闭环。先说明当前是诊断、探索性摘要草案还是投稿摘要；若未投稿就绪，先给修复方案，再按需要给标明不可投稿的草案。用“删减检验”取舍技术细节：仅保留会改变研究类型、验证角色、中心发现可信度或适用范围判断的信息，不用固定术语数、发现数或“R² 一律删除”代替判断。不要把 RQ 编号、模式名、诊断标签或占位符写进摘要。
```
