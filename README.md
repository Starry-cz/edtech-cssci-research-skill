...
| **证据判断服务写作** | 用“阻断—探索性可写—投稿就绪”控制结论强度；风险只限制相应承诺，不替代自然写作 | 发现测试集选模或泄漏后仍把未确认性能写成最终摘要、最优模型与机制结论 |
...
<details>
<summary><strong>摘要发布门：先判断是否允许写投稿摘要</strong></summary>

```text
使用 $edtech-cssci-research-skill 审查我的摘要生成条件。
请先在内部核对：研究问题、对象与情境、数据或设计、训练/验证/最终测试角色、最小发现、贡献与边界。
若存在测试集选模、泄漏、准则污染、结果不可复核或标题强度越界，不要把未确认性能或因果结论写成定稿事实；请给出自然的当前版本摘要，并在稿外简要列出会改变关键主张的重跑步骤和题目—摘要—图表—结论连锁更新。若我要求投稿判断，再明确给出“阻断 / 探索性可写 / 投稿就绪”状态。
材料：
[粘贴摘要、方法摘要或分析说明]
```
...
请先判定发布状态；没有决定性证据时不要帮我把结论写强，也不要用漂亮图表替代验证。
```
...
| [`assets/abstract-argument-card-template.md`](assets/abstract-argument-card-template.md) | 摘要承诺、选模状态、最小发现、贡献与边界的写前核对 | 想生成或重写投稿摘要前 |
| [`references/abstract-state-and-evidence-control.md`](references/abstract-state-and-evidence-control.md) | 诊断、探索性摘要草案与投稿摘要的区分；信息密度、证据锚点、预测动词与教育化概括的控制 | “摘要太技术化”“如何写得更像教育研究但不越界” |
...
| 全文诊断 | 总体判断 + 发布门 + `Pass / Partial / Fail` + 定位证据 + 表层泄漏 + 修改路径 | 确定阻断项、修订顺序和清洁稿放行条件 |
...
| 修订连锁 | 首要修复层 + 连锁更新 + 验证 + 状态 | 避免只改正文而遗漏摘要、图表、附录和投稿材料 |
