<p align="center">
  <img src="assets/readme-hero-v3.png" alt="教育技术学 CSSCI 研究助手：从真实教育问题出发，构建可追溯的研究论证链；覆盖选题诊断、文献综述、理论模型、实证分析和投稿自检" width="100%" />
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/41-研究参考模块-2563eb.svg" alt="41 个研究参考模块" />
  <img src="https://img.shields.io/badge/23-可复用模板-a16207.svg" alt="23 个可复用模板" />
</p>
...
适合情感计算、多模态大模型、教育人工智能与教育数据挖掘等方向：先明确教育问题与证据，再选择模型、写法和投稿策略。
...
| 8. 把实证分析写成教育技术学论文 | [实证论文论证闭环](references/education-technology-empirical-writing-closure.md) | “把模型比较、SHAP、ALE 和稳定性结果重组为教育问题、发现簇和讨论。” | 理论—构念—指标—证据—解释—启示闭环与章节重构顺序 |
| 9. 写或改某个论文部分 | [章节提示模板](assets/section-prompts/abstract.md) | “按实验研究写作需求，修改我的讨论：保持自然行文，分清结果、解释、贡献和边界。” | 对应章节的清洁正文、最小修改说明与必要证据提示 |
| 10. 按目标期刊改稿或投稿前检查 | [期刊画像与投稿核验](references/journals/index.md) | “按《电化教育研究》当前官方画像检查匿名、摘要和图表；未核验项列出即可。” | 期刊适配清单、待官方核验项、回应函/投稿包检查 |
...
| **论证先于润色** | 先锁定问题、回答、证据、贡献与边界，再写章节、改句子和回应审稿 | 先修辞后发现题目、摘要、图表和结论全部失配 |
| **论文写作与方法审计分离** | 默认先写教育问题、理论解释与中心发现；只有需要复现、设计检查或投稿前方法预审时才单列技术审计 | 把测试集、泄漏、稳定性和外部验证清单自动塞进摘要、讨论和结论 |
| **验证先于图表** | 每项主张都对应决定性证据、比较/反例、稳健性和边界 | 用一张漂亮框架图、显著性星号或模型排名承担全部论证 |
| **构思与投稿表层分离** | 内部保留结构标签、风险诊断和待办，交付时转换为连贯、自然、可投稿的学术正文 | 把“目的/方法/结果/结论”、RQ、模式名、诊断标签或占位符直接写进清洁稿 |
| **按需调用专项 Skill** | 先写清图、表或文档要服务的研究判断，再调用 Drawio、文档、PDF、表格等当前可用能力制作工件 | 为了美观补造机制图、因果箭头、数据图或无法追溯的排版成品 |
| **实证数据分析与建模一体化** | 从数据审计、测量、推断、多层/纵向分析，到防泄漏预测、样本外验证、SHAP/ALE 与稳定性检验 | 只报 *p* 值，或把相关、预测、特征重要性和模型表现误作因果效应 |
| **期刊适配而非格式猜测** | 指定八刊时优先读取官网画像卡、按文类加载章节功能模板，再检查匿名、摘要、图表与工件 | 编造期刊要求、参考文献、数据或审稿结果 |
...
已有材料：[填写]
目标期刊：[可选]
研究文类：[理论/综述/实验或准实验/设计型研究/质性或案例/混合方法/学习分析或可解释机器学习；可选]
...
| Claude Desktop / 网页版 | [Claude 适配说明](platforms/claude.md) | 使用 Project Instructions 与 Project Knowledge |
| Claude Code（本地） | [Claude Code 安装](platforms/claude.md) | 克隆仓库后由项目 `CLAUDE.md` 导入核心指令 |
| Gemini | [Gemini 适配说明](platforms/gemini.md) | 创建 Gem，并添加必要的 Knowledge 文件 |
| Trae（本地） | [Trae 安装](platforms/trae.md) | 直接克隆到项目根目录 `.agents/skills/` |

完整说明与按任务上传的知识文件组合见 [跨平台使用说明](platforms/platform-guide.md)。不同平台不会自动读取整个 GitHub 仓库；涉及某一专题时，请上传相关 `references/` 文件，或在对话中粘贴必要材料。Trae 与 Claude Code 的本地安装命令也已在该指南中集中说明。
...
| **写作与交付** | 搭建主论证、按文类写章节、按目标期刊改稿、全文重构、审稿回应、匿名与投稿包自检 | 论证工作底稿、章节草案、期刊适配清单、修订报告、回应函 |
...
| 4. 写作、诊断与修订 | 如何将证据写成可审查的论证，并修复核心缺陷 | 全文诊断、章节修订、修订连锁、审稿回应 | [写作工作流](references/writing-workflow.md) · [修订治理与语体诊断](references/revision-governance-and-style-diagnosis.md) · [章节模板](assets/section-prompts/abstract.md) · [论文论证与重构](references/academic-writing-and-revision.md) |
| 5. 投稿、工件与专项协作 | 稿件、图表、引文和投稿材料能否安全交付 | 工件质检、投稿前检查；按需调用绘图等其他 Skill | [期刊画像](references/journals/index.md) · [终稿自检](references/self-review.md) · [跨 Skill 工件路由](references/cross-skill-artifact-routing.md) |
...
| **贡献与边界相称** | 按提出、描述、关联、预测、检验或设计原则校准结论强度 | 把系统上线、案例、政策目标或显著性写成普遍教育效果 |
...
| 阶段 | 主要任务 | 典型交付物 | 对应文件 |
|---|---|---|---|
| 1. 选题与问题意识 | 识别真实教育矛盾、对象、场景、范围、证据与伦理风险；将热点转为可回答问题 | 七道质量门、候选选题、主问题/子问题、证据需求 | [选题诊断](references/topic-diagnosis-and-research-questions.md) · [选题画布](assets/topic-question-evidence-canvas.md) |
...
| 4. 研究设计与数据 | 选择范式，界定样本、测量、数据生成、分析单位、清洗、统计/预测模型、主张验证与识别条件 | 研究设计、数据字典、分析计划、主张验证矩阵、伦理与权限清单 | [研究范式](references/research-paradigms.md) · [教育数据分析](references/education-data-analysis.md) · [统计报告与图表证据门](references/statistical-reporting-and-figure-evidence.md) · [可解释机器学习](references/interpretable-machine-learning.md) · [主张验证模板](assets/claim-evidence-validation-matrix-template.md) |
| 5. 结果、讨论与写作 | 先搭主论证，再组织题目、摘要、引言、综述、方法、结果、讨论和结论；按需制作图表工件 | 论证工作底稿、摘要论证卡、清洁正文章节、方法审计说明（仅按需）、图表说明书、章节结尾 | [论文论证与重构](references/academic-writing-and-revision.md) · [实证论文论证闭环](references/education-technology-empirical-writing-closure.md) · [写作工作流](references/writing-workflow.md) · [投稿正文与表层控制](references/publication-prose-and-style-control.md) · [八刊写作证据台账](references/eight-journal-writing-evidence.md) · [摘要论证卡](assets/abstract-argument-card-template.md) · [跨 Skill 工件路由](references/cross-skill-artifact-routing.md) |
| 6. 诊断、返修与回应 | 全文诊断、框架防御、多审稿人意见处理、修订连锁与版本传播检查 | P0/P1/P2 修订计划、回应函、修改台账 | [修订治理与语体诊断](references/revision-governance-and-style-diagnosis.md) · [终稿自检](references/self-review.md) · [审稿回应](references/revision-and-reviewer-response.md) · [修订台账模板](assets/reviewer-response-and-revision-ledger-template.md) |
...
| 模式 | 适用任务 | 核心产出 |
|---|---|---|
| `topic_diagnosis` | 热点、政策或实践困扰的七道质量门诊断 | 选题判断、聚焦方向、候选题目与证据缺口 |
| `research_question_refinement` | 把主题改成主问题与相互依赖子问题 | 分析单位、证据、方法方向与结论边界 |
| `outline_building` | 新建或修复论文结构 | 章节—功能—证据—问题对应表 |
| `argumentation_blueprint` | 搭建全文主线、核对贡献与结论边界 | 问题—回答—证据—贡献—边界主张图 |
...
| `revision_cascade` | 分析结构性修改对摘要、图表、附录和回应函的影响 | 修改层级、连锁更新、验证与状态 |
...
| `reference_and_artifact_audit` | 参考文献、匿名稿、图表、DOCX/PDF | 投稿工件检查与关键缺口 |
...
<details>
<summary><strong>按某刊改稿：先读画像，再给自然正文</strong></summary>

```text
使用 $edtech-cssci-research-skill，按《[目标期刊]》修改我的[摘要/引言/方法/讨论/整稿]。

研究文类：[理论/综述/实验或准实验/设计型研究/质性或案例/混合方法/学习分析或可解释机器学习]
请先读取该刊官方画像卡和本节提示模板：只采用已核验的官方要求；画像超过90天、缺少细则或未获得官方文件时，列为“待官方核验”，不要杜撰格式。随后输出自然、可继续修订的正文，并把工件核对另列为清单。
稿件：
[粘贴]
```
...
<details>
<summary><strong>摘要改写：先形成教育学中心判断</strong></summary>

```text
使用 $edtech-cssci-research-skill 改写我的教育技术学论文摘要。
请先在内部归并教育问题、对象情境、研究行动、核心发现和中心判断。若使用机器学习，请把变量先归并为教育构念与过程维度；用“问题—研究行动—发现—意义”组织自然摘要，避免把算法、测试集、解释器和重跑事项写成摘要主体。
只有我明确要求方法审计或投稿前核验时，才在稿外说明会影响“独立最终性能、严格阈值或因果效果”等单项承诺的问题与修复路径。
材料：
[粘贴摘要、方法摘要或分析说明]
```
...
<details>
<summary><strong>预测研究预检：SHAP/ALE 之前先检查验证设计</strong></summary>

```text
使用 $edtech-cssci-research-skill 预检我的预测研究。
请审计目标变量与特征重叠、分析单位、群组/时间切分、折内预处理、训练—验证—最终测试分工、选模规则、解释模型锁定、SHAP相关特征、ALE稀疏区与不确定性。
先给出风险判断；若同一测试集参与20个模型比较与最终报告，说明其不能支持“独立最终最优表现”，并给可执行的重跑方案。不要把特征归因直接写成因果机制或行动处方。
```

</details>

<details>
<summary><strong>主张验证与图表契约：避免“图好看、证据不够”</strong></summary>

```text
使用 $edtech-cssci-research-skill，将我的论文关键结论做成“主张—决定性证据—比较/反例—稳健性—边界”矩阵。
再为每张关键图填写图表证据契约：核心结论、面板任务、证据层级、样本/单位、统计或不确定性、图注信息和审稿风险。
请指出每项主张还需要的最小证据；不要用漂亮图表替代验证，也不要把核验台账直接写入论文正文。
```

</details>

<details>
<summary><strong>八刊风格诊断：学习组织规则，不模仿句式</strong></summary>

```text
使用 $edtech-cssci-research-skill，按八刊样本库诊断下面稿件。
先判断我的研究类型，再只提取同类文章可迁移的规则：问题入口、理论—方法接口、结果—讨论分工、图表功能与摘要信息密度；同时列出不应模仿的强结论或句式。
不要判断八刊当前CSSCI身份、不要预测录用，也不要直接改写成某位作者的风格。
稿件：
[粘贴]
```
...
<details>
<summary><strong>论文主论证与全文重构</strong></summary>

```text
使用 $edtech-cssci-research-skill，为我的教育技术学论文建立主论证图。

请按“真实问题—研究问题—暂定回答—理论/构念—设计与证据—最小贡献—实践含义—结论边界”输出，并为题目、摘要、引言、综述、方法、结果、讨论和结论分配一个最小判断。没有材料支持的部分标为待补材料，不要编造文献、数据或结果。
```

</details>

<details>
<summary><strong>摘要与投稿正文表层审查</strong></summary>

```text
使用 $edtech-cssci-research-skill，审查并修改下面这份摘要。

先核验目标期刊是否明确要求结构式摘要；若没有，不在清洁稿中保留“目的/方法/结果/结论”等构思标签。检查题目强度是否匹配研究设计，预测结果是否被误写为因果效应，SHAP/ALE 是否被拼成模型未直接观察到的过程闭环。内部判断当前证据状态；除非我要求投稿判断或风险会改变关键主张，否则直接给自然改写，不附“不可投稿”标签。用“删减检验”取舍技术细节：仅保留会改变研究类型、验证角色、中心发现可信度或适用范围判断的信息，不用固定术语数、发现数或“R² 一律删除”代替判断。区分机制、影响等词是在研究问题、理论解释还是实证结论中使用；不要把 RQ 编号、模式名、诊断标签或占位符写进摘要。
```

</details>

<details>
<summary><strong>研究框架图与 Drawio 协作</strong></summary>

```text
使用 $edtech-cssci-research-skill，为这项教育技术学研究制作研究框架图。

先输出图表说明书：图的唯一判断、节点定义、箭头含义、每项关系的理论或材料依据、尚待检验的路径、图题图注和正文放置位置。确认后，调用当前可用的 Drawio Skill 制作可编辑图，并导出我指定的格式。不要把并列变量补画成因果链、反馈闭环或中介机制；完成后回检图内术语、箭头、数值、图注与正文结论的一致性。
```
...
拆分复合意见，区分共识、互补、冲突和独有问题；为每条意见给出 accept、partial、decline 或 needs verification，写明首要修复层、修改位置、连锁更新和验证方式，并生成回应函和修订台账。
```
...
| [`scripts/audit_skill.py`](scripts/audit_skill.py) | 无依赖一致性审计：元数据、资源路由、相对链接、模式与数量声明 | 修改 Skill 或 README 后 |
...
| [`references/topic-diagnosis-and-research-questions.md`](references/topic-diagnosis-and-research-questions.md) | 选题七道质量门、热点张力、问题类型、专家论文结构观察与报告标准倒推 | “热点如何变选题”“研究问题能否回答” |
| [`references/academic-writing-and-revision.md`](references/academic-writing-and-revision.md) | 主论证链、贡献与边界、全文重构、修订连锁检查 | “主线怎么搭”“论文怎么重构”“改动会影响哪里” |
| [`references/education-technology-empirical-writing-closure.md`](references/education-technology-empirical-writing-closure.md) | 理论—构念—指标—证据—解释—启示闭环；实证结果与讨论分工 | “模型结果像数据报告”“框架、变量和讨论如何闭合” |
| [`references/writing-workflow.md`](references/writing-workflow.md) | 题目、摘要、引言、综述、理论、方法、结果、讨论与结论 | “帮我写/改这一部分” |
| [`assets/section-prompts/`](assets/section-prompts/abstract.md) | 摘要、引言、综述、方法、结果、讨论、结论七类章节的输入、功能判断与输出契约 | “按本节功能写”“不要模板腔” |
| [`references/functional-phrasing-bank.md`](references/functional-phrasing-bank.md) | 问题、缺口、理论、设计、结果、解释、边界与建议的功能性表达及误用风险 | “这句话怎么更准确，但不要模仿作者” |
| [`references/publication-prose-and-style-control.md`](references/publication-prose-and-style-control.md) | 内部构思与投稿正文分离、摘要标签、标题强度、机制越界与清洁稿放行 | “摘要太像模板”“只给可投稿正文”“检查内部标签泄漏” |
| [`references/claim-evidence-validation-contract.md`](references/claim-evidence-validation-contract.md) | 关键主张、决定性证据、比较/反例、稳健性和边界的统一验证包 | “每个结论究竟靠什么证据成立” |
| [`references/statistical-reporting-and-figure-evidence.md`](references/statistical-reporting-and-figure-evidence.md) | 独立单位、多重比较、效应量/不确定性与图注统计报告 | “检查统计分析和图注” |
| [`references/source-grounded-paper-reading.md`](references/source-grounded-paper-reading.md) | 溯源阅读、图文对应、可定位笔记与术语表 | “精读这篇论文，不要只给摘要式总结” |
| [`references/pre-submission-peer-review.md`](references/pre-submission-peer-review.md) | 共同事实底稿、三视角预审和风险综合 | “模拟预审，但不要虚构审稿人” |
| [`references/eight-journal-writing-evidence.md`](references/eight-journal-writing-evidence.md) | 八刊样本台账、可迁移的摘要/论证/图表规则与风格诊断边界 | “按八刊同类文章诊断，不模仿作者句式” |
...
| [`references/cross-skill-artifact-routing.md`](references/cross-skill-artifact-routing.md) | 图、表、DOCX/PDF、工作簿和配图的按需专项 Skill 调用与回检 | “需要画图”“生成 Word/PDF”“制作数据表” |
| [`references/external-evidence-and-research-positioning.md`](references/external-evidence-and-research-positioning.md) | 公开期刊范围、国际报告与学术共同体资料如何转为问题、条件和边界 | “技术很热如何形成问题”“如何说明研究意义”“AI治理条件” |
...
| [`references/interpretable-machine-learning.md`](references/interpretable-machine-learning.md) | 将预测、SHAP/ALE 转译为教育研究发现；按需进行防泄漏与验证审计 | “写机器学习论文”“做可解释机器学习”“检查建模方法” |
...
| [`references/revision-governance-and-style-diagnosis.md`](references/revision-governance-and-style-diagnosis.md) | P0/P1/P2 修订顺序与反模板语体诊断 | “全文怎么改”“像报告或 AI 腔”“这一轮先处理什么” |
...
| [`references/journals/index.md`](references/journals/index.md) | 八刊官方画像卡、核验日期、适用范围与待核验项 | “按某刊改稿”“投稿前查格式” |
| [`scripts/check_journal_profile.py`](scripts/check_journal_profile.py) | 按画像检查摘要、关键词、匿名、图表等可识别信号 | “跑一次期刊投稿工件预检” |
| [`scripts/check_section_function.py`](scripts/check_section_function.py) | 按文类检查章节功能候选缺口 | “检查引言、方法、结果和讨论是否混写” |
...
| [`assets/topic-question-evidence-canvas.md`](assets/topic-question-evidence-canvas.md) | 选题、问题、对象、机制、证据与边界画布 | 从热点、政策或实践困扰形成研究设计前 |
| [`assets/revision-report-template.md`](assets/revision-report-template.md) | 全文诊断与修订报告 | 已有完整草稿，需要确定修改优先级时 |
| [`assets/argumentation-and-revision-workbook-template.md`](assets/argumentation-and-revision-workbook-template.md) | 主论证、章节功能、主张—证据—边界与修订传播 | 写作前搭主线或结构性返修时 |
| [`assets/manuscript-surface-audit-template.md`](assets/manuscript-surface-audit-template.md) | 投稿正文放行、摘要表层与内部痕迹审计 | 从论证底稿生成清洁稿或投稿前终审时 |
| [`assets/claim-evidence-validation-matrix-template.md`](assets/claim-evidence-validation-matrix-template.md) | 关键主张与决定性证据、反例、稳健性、边界的矩阵 | 研究设计、写作与终稿审查时 |
| [`assets/figure-evidence-contract-template.md`](assets/figure-evidence-contract-template.md) | 制图前的核心结论、面板任务、统计与审稿风险契约 | 绘制框架图、流程图或结果图之前 |
| [`assets/abstract-argument-card-template.md`](assets/abstract-argument-card-template.md) | 问题、研究行动、发现簇与中心判断的写前压缩 | 想生成或重写投稿摘要前 |
| [`assets/empirical-writing-closure-canvas.md`](assets/empirical-writing-closure-canvas.md) | 实证论文的框架闭合、证据阶梯与章节推进 | 写作前整理课堂观察、学习分析或机器学习研究时 |
| [`references/abstract-state-and-evidence-control.md`](references/abstract-state-and-evidence-control.md) | 将方法报告压缩为教育技术学摘要；方法审计与摘要写作的分离 | “摘要太技术化”“如何写得更像教育研究” |
| [`assets/problem-model-evidence-canvas.md`](assets/problem-model-evidence-canvas.md) | 问题—模型—证据—实践画布 | 构建或审查概念模型、实践模式时 |
| [`assets/research-positioning-evidence-matrix-template.md`](assets/research-positioning-evidence-matrix-template.md) | 教育问题、外部定位、设计、证据、治理与边界矩阵 | 以报告、期刊范围或趋势定位研究时 |
...
| [`examples/writing/`](examples/writing/abstract-revision.md) | 合成摘要、方法、讨论与审稿回应的强/弱/修订对照 | 学习论证功能，不模仿真实作者或期刊原文 |
...
| 任务 | 默认输出 | 最终用途 |
|---|---|---|
| 正文写作 | 不含内部标签和占位符的清洁正文 + 分离的修改说明与证据边界 | 形成可继续修改或投稿适配的章节草稿 |
| 论文主论证 | 问题—回答—证据—贡献—边界 + 章节功能 + 主张矩阵 | 在写作前锁定全文主线，避免章节拼接 |
| 结构设计 | 章节/方案 + 功能、证据和研究问题对应关系 | 对齐论文大纲、研究问题与材料 |
| 文献任务 | 检索策略 + 来源状态 + 筛选日志 + 阅读笔记/矩阵 + 引文风险 | 保证综述来源可追溯 |
| 全文诊断 | P0/P1/P2 修订优先级 + 定位证据 + 反模板语体诊断 + 修改路径 | 确定关键缺口、修订顺序和清洁稿放行条件 |
| 图表与研究工件 | 工件说明书 + 专项 Skill 路由 + 可编辑交付要求 + 图文一致性回检 | 生成可追溯、可复核且不越界的研究图表或文件 |
| 框架审查 | 主张—依据—竞争解释—操作化—边界—图示对应 | 支撑模型图与正文的一致性 |
| 审稿返修 | 意见矩阵 + 处理决定 + 修改位置 + 回应函 + 修订台账 | 管理多轮返修与版本变化 |
| 修订连锁 | 首要修复层 + 连锁更新 + 验证 + 未决事项 | 避免只改正文而遗漏摘要、图表、附录和投稿材料 |
| 教育数据分析 | 数据审计 + 分析计划 + 描述/测量/模型结果 + 诊断、稳健性与复现边界 | 形成可复现、可解释的分析报告 |
| 可解释机器学习 | 数据流程 + 模型比较 + 解释图表 + 稳定性 + 复现与边界 | 避免把预测结果误写为因果结论 |
| 投稿前检查 | 重大缺口 + 主要修改 + 润色项 + 提交判断 | 完成目标期刊的最终交付质检 |
...
<details>
<summary><strong>研究质量与终稿规则（按需展开）</strong></summary>

<br />

- 学习分析与可解释机器学习摘要先将变量归并为教育构念和过程维度，再形成有理论依据的解释；验证风险只限制“最终最优”“严格阈值”“净因果”等相应承诺，不把摘要写成方法审计说明书。
- 写作按章节分工：摘要压缩研究判断，结果报告证据，讨论完成教育解释；局限只说明会实质改变结论的条件，结论回到题目形成最小推进。
- 要求“最终版、可直接投稿”时，编码、授权、伦理、来源与作者等事实必须齐备；不得以“投稿时补充”“应如实说明”或“待完善”等文字填补缺失事实。
