...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/29-研究参考模块-2563eb.svg" alt="29 个研究参考模块" />
  <img src="https://img.shields.io/badge/15-可复用模板-a16207.svg" alt="15 个可复用模板" />
</p>
...
| 摘要要像投稿摘要，但担心承诺超过数据和设计 | [摘要论证卡](assets/abstract-argument-card-template.md) · [摘要状态控制](references/abstract-state-and-evidence-control.md) | 先区分诊断、探索性摘要草案与投稿摘要；再核对问题、对象、设计、锁定模型、最小发现、贡献与边界 |
...
| **发布门先于定稿** | 用“阻断—探索性可写—投稿就绪”决定当前可生成的文本；阻断时将修复诊断与不可投稿的探索性草案分开 | 发现测试集选模或泄漏后仍写最终摘要、最优模型与机制结论 |
...
请先给出“阻断 / 探索性可写 / 投稿就绪”状态，并核对：研究问题、对象与情境、数据或设计、训练/验证/最终测试角色、最小发现、贡献与边界。
若存在测试集选模、泄漏、准则污染、结果不可复核或标题强度越界，不要生成投稿摘要；请先给重跑步骤和题目—摘要—图表—结论的连锁更新。若我仍需要修改当前文本，再附一段明确标注“当前分析版本、不可投稿”的探索性摘要草案。只有投稿就绪时才给一段连续的中文投稿摘要。
材料：
[粘贴摘要、方法摘要或分析说明]
```
...
先给发布状态；若同一测试集参与20个模型比较与最终报告，明确阻断并给可执行的重跑方案。不要把特征归因写成因果机制或行动处方。
```
...
先核验目标期刊是否明确要求结构式摘要；若没有，不在清洁稿中保留“目的/方法/结果/结论”等构思标签。检查题目强度是否匹配研究设计，预测结果是否被误写为因果效应，SHAP/ALE 是否被拼成未经观测的过程闭环。先说明当前是诊断、探索性摘要草案还是投稿摘要；若未投稿就绪，先给修复方案，再按需要给标明不可投稿的草案。不要把 RQ 编号、模式名、诊断标签或占位符写进摘要。
```
...
| [`references/abstract-state-and-evidence-control.md`](references/abstract-state-and-evidence-control.md) | 诊断、探索性摘要草案与投稿摘要的区分；信息密度、预测动词与教育化概括的证据控制 | “摘要太技术化”“如何写得更像教育研究但不越界” |
